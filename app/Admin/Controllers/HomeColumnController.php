...
{
    private $createUrl = "/home/column/create?";
...
//                ->params(['id'=>'11','name1'=>'name'])
    ...
    {
        $grid->showSummary(true);
        $grid->height(400);
        $grid->dialogForm($this->form()->isDialog());
        $grid->addDialogForm($this->form()->isDialog());
        $grid->editDialogForm($this->form(true)->isDialog());

        $grid->column('sort', '排序')->prop('sort')->component(
            SortUpDown::make(110)->setSortAction(config('deep_admin.route.api_prefix') . '/entities/content/sort_up_down?entity_id=54')
        );
...
//        $grid->column('is_show_content_manage', "状态")->width(100)->align("center")->component(Boole::make());
    ...
    public function relation_grid(\Andruby\DeepAdmin\Layout\Content $content, $home_column_id = null)
    {
        ...
        $content->row(function (Row $row) use ($grid_left, $grid_right, $homeColumnInfo) {
            ...
            $row->column(12, Card::make()->header('栏目：' . $homeColumnInfo['name'] . '，已关联内容')->bodyStyle(['padding' => '0', 'margin_left' => '100'])->content(
                $grid_left
            ));
            $row->column(12, Card::make()->header('专栏列表')->bodyStyle(['padding' => '0'])->content(
                $grid_right
            ));
        });
        if ($grid_type == 1) {
    ...
    private function column_grid($grid_type, $table_name, $home_column_id)
    {
        ...
        debug_log_info('$grid_type = ' . $grid_type);
        $grid->height(400);
        $grid->toolbars(function (Grid\Toolbars $toolbars) {
            $toolbars->hideCreateButton();
        });

        $grid->quickSearch(['title'])->quickSearchPlaceholder("专栏名称");
        ...
        $grid->column('title', "专栏名称");
        $grid->column('column_type', '类型')->customValue(function ($row, $value) {
            $values[0] = '专栏';
            $values[1] = '大专栏';
            return $values[$value];
        })->component(function () {
        ...
        if ($grid_type == 1) {
            $grid->column('sort', '排序')->component(
                SortEdit::make()->action(config('deep_admin.route.api_prefix') . '/entities/content/grid_sort_change?entity_id=55')
            )->width(80)->sortable();
        }
    ...
    public function relation()
    {
        ...
        if ($grid_type == 1) {
            ...
            if ($count > 0) {
                return \Admin::responseError('专栏已关联');
            } else {
                $item['home_column_id'] = $home_column_id;
                $item['column_id'] = $column_id;
                $item['updated_at'] = date('Y-m-d H:i:s', time());
                ...
                if ($id) {
                    $data['action']['emit'] = 'tableReload';
                    return \Admin::response($data, '专栏关联成功');
                } else {
                    return \Admin::responseError('专栏关联失败，操作异常');
                }
            }
        }
